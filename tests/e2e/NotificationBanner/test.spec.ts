...
test.describe('NotificationBanner', () => {
    ...
    async function saveSettingsAndWait(page: any) {
        await page.getByRole(SELECTORS.saveButton.role, { name: SELECTORS.saveButton.name }).click();
        await Promise.race([
            page.waitForURL(/admin\.php\?page=dswp-notification-menu.*settings-updated/i, { timeout: 5000 }).catch(() => { }),
            page.locator('.notice-success, .updated.notice').first().waitFor({ state: 'visible', timeout: 5000 }).catch(() => { }),
            page.waitForLoadState('networkidle', { timeout: 5000 }).catch(() => { }),
        ]);
    }

    async function enableBanner(page: any) {
        await page.getByRole(SELECTORS.enableRadio.role, { name: SELECTORS.enableRadio.name }).check();
    }

    async function disableBanner(page: any) {
        await page.getByRole(SELECTORS.disableRadio.role, { name: SELECTORS.disableRadio.name }).check();
    }
    ...
    test.describe('Visibility', () => {
        ...
        test('Banner should not render if it has empty content', async ({ page }) => {
    ...
    test('Banner should display with message and each color', async ({ page }) => {
        for (const color of BANNER_COLORS) {
            ...
            const frontend = await visitFrontend(page);
            await expect(frontend.locator(`text=${message}`)).toBeVisible();
            const banner = await assertBannerVisible(frontend);
            await expect(banner).toHaveAttribute(
                'style',
